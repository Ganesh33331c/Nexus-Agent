...
def _resolve_model() -> str:
    ...
    try:
        ...
        print(f"[NEXUS][MODEL] Available generateContent models: {sorted(available)}")

        for candidate in _PREFERRED_MODELS:
            if candidate in available:
                _resolved_model = candidate
                print(f"[NEXUS][MODEL] Selected model: {_resolved_model}")
                return _resolved_model
        ...
        _resolved_model = _PREFERRED_MODELS[0]
        print(f"[NEXUS][MODEL] ListModels failed ({e}) — defaulting to {_resolved_model}")
        return _resolved_model
